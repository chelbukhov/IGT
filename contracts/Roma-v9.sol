// оставлен только один адрес команды
...
contract Crowdsale is Ownable {
    ...
    constructor() public {
        ...
        uint sendTokens = 5250000 * 1 ether;
        // передача токенов на хранение на адрес команды (10%)
        giveTokens(address(commandAddress), sendTokens);
    ...
    function calcBonus () public view returns(uint256) {
        ...
        if (amountToken >= 2240000 && amountToken < 4400000){
            actualBonus = 8;
        }
        if (amountToken >= 4400000 && amountToken < 6520000){
            actualBonus = 6;
        }
        if (amountToken >= 6520000 && amountToken < 8600000){
            actualBonus = 4;
        }
        if (amountToken >= 8600000 && amountToken < 10640000){
            actualBonus = 2;
        }
